...
<div class="wskyscraper">
    ...
    <? if (User::isDonator() || $this->get('ad_force_donator')) : ?>
        var ad_inject = '<?= $this->generateURL('Advertise.Main', array('size' => 'wskyscraper')) ?>';
    ...
    <? else : ?>
		<? if ($this->get('ad_network') == 'doubleclick') : ?>
			var ad_inject = 'http://<?= MAIN_SERVER ?>/advertise/doubleclick.php?size=wskyscraper&campaign=<?= $this->get('ad_campaign') ?>&t=<?= $this->get('ad_type') ?>&s=<?= $this->get('ad_site') ?>&z=<?= $this->get('ad_zone') ?>&a=<?= $this->get('ad_age') ?>&g=<?= $this->get('ad_gender') ?>&o=<?= $this->get('ad_bc_time') ?>';
		<? else : ?>
        	var ad_inject = 'http://<?= MAIN_SERVER ?>/advertise/<?= $this->get('ad_network') ?>.php?size=wskyscraper&campaign=<?= $this->get('ad_campaign') ?>';
        <? endif; ?>
    <? endif; ?>
	/* ]]> */
    </script>
